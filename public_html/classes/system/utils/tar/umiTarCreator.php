<?php
	...
	class umiTarCreator {
		...
		private function writeHeader($name, $stat, $type = '0') {
			$userInfo = function_exists('posix_getpwuid') ? posix_getpwuid($stat['uid']) : ['name' => ''];
			$groupInfo = function_exists('posix_getgrgid') ? posix_getgrgid($stat['gid']) : ['name' => ''];
			...
			if (mb_strlen($name) > 100) {
				$splitPos = mb_strpos($name, '/', mb_strlen($name) - 100);
				$prefix = mb_substr($name, 0, $splitPos);
				$name = mb_substr($name, $splitPos + 1);
			}
			$header = pack(
				'a100a8a8a8a12a12a8a1a100a6a2a32a32a8a8a155x12',
				$name,
				$mode,
				$uid,
				$gid,
