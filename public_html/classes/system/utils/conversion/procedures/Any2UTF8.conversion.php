	...
	class Any2UTF8 implements IGenericConversion {
		...
		private static function detectCharset($sStr) {
			...
			foreach ($arrCyrEncodings as $iCyrEnc => $sCyrEnc) {
				...
				if ($sTestStr && (mb_strlen($sStr) / mb_strlen($sTestStr)) <= 3) {
					$arrTestRezs[$sCyrEnc] =
						['badtrios' => 0, 'badpairs' => 0, 'badchars' => 0, 'goodchars' => 0, 'length' => 0, 'goodindex' => 0];
					...
					for ($i = 0; $i < (mb_strlen($sTestStr) - 1); $i++) {
						$iCh0 = ord($sTestStr[$i]);
						if ($iCh0 < 0x20) {
							$arrTestRezs[$sCyrEnc]['badchars'] += 1;
						}
						if (in_array(mb_substr($sTestStr, $i, 2), $arrNonexWinRu2)) {
							$arrTestRezs[$sCyrEnc]['badpairs'] += 1;
						}
						if (in_array(mb_substr($sTestStr, $i, 3), $arrNonexWinRu3)) {
							$arrTestRezs[$sCyrEnc]['badtrios'] += 1;
						}
					}
					//
					$iGoodChars = (int) preg_match_all("/[\xE0-\xFF\x61-\x7A\-_ \"']/is", $sTestStr, $arrTmp);
					$iAllChars = mb_strlen($sTestStr);
			...
			foreach ($arrTestRezs as $sCyrEnc => $arrEncRezs) {
				if ($arrEncRezs['goodindex'] === 100 ||
					($arrEncRezs['badchars'] === 0 && $arrEncRezs['badpairs'] === 0 && $arrEncRezs['badtrios'] === 0)) {
					if (!isset($arrMbValids[$arrEncRezs['goodindex']])) {
						$arrMbValids[$arrEncRezs['goodindex']] = $sCyrEnc;
					}
				}
			}
			//
			if (umiCount($arrMbValids)) {
				krsort($arrMbValids);
