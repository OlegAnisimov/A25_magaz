<?php
	...
	final class antiSpamHelper {
		...
		private static function fillFields(
			...
			if ($author && $author->is_registrated) {
				$author = $objects->getObject($author->user_id);
			}
			if ($author) {
				$nick = $author->nickname ?: $author->login;
				$email = $author->email ?: $author->getValue('e-mail');
			...
			$link = Service::DomainDetector()->detectHost() . $hierarchy->getPathById($element->getId());
