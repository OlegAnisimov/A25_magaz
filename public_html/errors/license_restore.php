<?php
	...
	$defaultDomainHost = Service::DomainCollection()
		->getDefaultDomain()
		->getHost();
?>
...
function checkSystem() {
	...
	jQuery.get('/errors/save_domain_keycode.php', {'keycode':keycode}, function(response) {
		...
		if (isDomainNotDefault[0].textContent === '1') {
			document.getElementById('license_msg').innerHTML = 'Для активации системы перейдите на основной домен <?= $defaultDomainHost ?>.';
...
function checkLicenseCode(frm) {
	...
	<?php $serverAddress = Service::Request()->serverAddress(); ?>
	var ip = "<?= isset($serverAddress) ? $serverAddress : str_replace("\\", '', Service::Request()->documentRoot()); ?>";
